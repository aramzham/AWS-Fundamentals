    ...
    public void FunctionHandler(DynamoDBEvent dynamoEvent, ILambdaContext context)
    {
        ...
        foreach (var record in dynamoEvent.Records)
        {
            ...
            if (record.Dynamodb.OldImage != null)
            {
                ...
                context.Logger.LogInformation($"Event Body Old Image: {oldDocument.ToJsonPretty()}");
            }

            var newDocument = Document.FromAttributeMap(record.Dynamodb.NewImage);
            context.Logger.LogInformation($"Event Body New Image: {newDocument.ToJsonPretty()}");
        }
